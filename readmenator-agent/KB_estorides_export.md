...
- Layer: utility
- Doc: estorides_export.encryption =========================== Optional age-encrypted report export.  The default export path p
- Language: py
- Symbols:
...
- Layer: utility
- Doc: estorides_export.misp ===================== Minimal MISP event JSON. Mirrors STIX in spirit but matches the MISP galaxy/
- Language: py
- Symbols:
...
- Symbols:
  ...
  - `build_executive_summary` (method, line 92) `def build_executive_summary(critical_findings, total_targets, domain, classification)`
  - `generate_report` (method, line 115) `def generate_report(query, target_scoring, metadata)`
...
- Layer: utility
- Doc: estorides_export.report =======================  Markdown report generation for finished cases.  Design choices --------
- Language: py
- Symbols:
  ...
  - `_analysis` (function, line 158) `def _analysis(case)`
  - `_meta_footer` (function, line 179) `def _meta_footer(case, sources_queried, sources_succeeded)`
  - `render_markdown_report` (function, line 196) `def render_markdown_report(case, entities, sources_queried, sources_succeeded, diff)`
...
- Layer: utility
- Doc: estorides_export.stix ===================== STIX 2.1 bundle export. Each unique entity becomes a STIX object (identity, 
- Language: py
- Symbols:
