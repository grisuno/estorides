...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.active_recon.  Implements the Given-When-Then contracts declared in ``spec/active_re
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for egress anonymisation wiring (`estorides_core.async_client`).  Ported from the standalone ``_test_proxy.py`
- Language: py
- Symbols:
...
- Layer: testing
- Doc: Auth gate: bearer-token protection for sensitive /api/* routes.  When `ESTORIDES_AUTH_TOKEN` is set, the gate is on: ano
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.change_detection.  Implements the Given-When-Then contracts declared in ``spec/chang
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD / regression tests for the `estorides watch add` CLI path.  Context (boy-scout fix): `cmd_watch_add` used to crash w
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.cloud_asset_discovery.  Implements the Given-When-Then contracts declared in ``spec/
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.code_exposure.  Implements the Given-When-Then contracts declared in ``spec/code_exp
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD / regression tests for fault-tolerant configuration parsing.  Doctrine (`config.py` docstring): a malformed env var 
- Language: py
- Symbols:
...
- Layer: testing
- Doc: CSP-safe styles: regression tests for the inline-style bug introduced by the CSP tightening in commit 45c3af5.  The cont
- Language: py
- Symbols:
...
- Layer: testing
- Doc: Encrypted export: plaintext must be removed after encryption.  Issue #8 reported that `export_stix_encrypted` and `expor
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD / regression tests for entity extraction and fuzzy merge.  - EE1: `types=[]` extracts nothing (and `types=None` extr
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for estorides_core.entity_resolution.  These tests implement the Given-When-Then contracts declared in spec/en
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.fusion_analytics.  Implements the Given-When-Then contracts from spec/fusion_analyti
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for the v1.3 hardening surface, case diff and report.  Ported from the standalone ``_test_hardening.py`` valid
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.hypothesis_engine.  Implements the Given-When-Then contracts declared in ``spec/hypo
- Language: py
- Symbols:
...
## tests/test_ids.py
- Layer: testing
- Doc: BDD tests for the shared deterministic id helper (spec/ids.md).  - ID1: deterministic and content-addressed - ID2: lengt
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for Monitoring & Advanced Recon modules.  Covers: - M1: Create a watch target - M2: Watch runs and completes -
- Language: py
- Symbols:
...
- Layer: testing
- Doc: estorides_core.observation_models — BDD/TDD contract tests.  Each test is the executable translation of a BDD scenario i
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for operator-OPSEC contact classification + passive-only filter.  Ported from the standalone ``_test_passive.p
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for estorides_core.pagination.  These tests verify the pagination strategy config, param building, cursor extr
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD / regression tests for the remote-JSON parser layer.  Doctrine (`parsers.py` header): every parser MUST be total — a
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.pdns_monitor.  Implements the Given-When-Then contracts declared in ``spec/pdns_moni
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.people_intel.  Implements the Given-When-Then contracts declared in ``spec/people_in
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD + property tests for probabilistic fusion.  These tests verify that :meth:`FusionStore.fuse_entity` and :meth:`Fusio
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for the recon_fusion module (Modulo 2g).  Each scenario maps to a Given-When-Then from spec/recon_fusion.md.
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_export.recon_report.  Implements the Given-When-Then contracts declared in ``spec/recon_r
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.reliability_scoring.  These tests implement the Given-When-Then contracts declared i
- Language: py
- Symbols:
...
## tests/test_scope.py
- Layer: testing
- Doc: BDD tests for the bug-bounty scope classifier (`estorides_core.scope`).  Ported from the standalone ``_test_scope.py`` v
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD/ATDD suite for the `search_telemetry` module (spec/search_telemetry.md).  Each test maps to a Given-When-Then scenar
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for SOCMINT sources and SocialMediaInferer.  Covers: - S1: YouTube channel lookup (happy path) - S2: YouTube c
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for estorides_core.source_health_monitoring.  These tests implement the Given-When-Then contracts declared in 
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD / regression tests for SourceRegistry loading.  - SL1: a multi-document YAML file loads every document - SL2: a docu
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for the shared SQLite store base (spec/sqlite_store.md).  - SS1: schema applied, default path honoured - SS2: 
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for structured human-selector extraction + pivot leaf surfacing.  Ported from the standalone ``_test_people.py
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.supply_chain.  Implements the Given-When-Then contracts declared in ``spec/supply_ch
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for estorides_core.system_app_sources.  Kali OSINT CLI tools as first-class `system_app` sources: same aggrega
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.target_scoring.  Implements the Given-When-Then contracts declared in ``spec/target_
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.tech_fingerprint.  Implements the Given-When-Then contracts declared in ``spec/tech_
- Language: py
- Symbols:
...
- Layer: testing
- Doc: Tests for estorides_core.tool_install (lazyaddon-style tool installation).  Covers recipe loading, elevation choice (run
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.tool_runner.  Implements the Given-When-Then contracts declared in ``spec/tool_runne
- Language: py
- Symbols:
...
- Layer: testing
- Doc: Regression tests for the `hidden` attribute contract and output escaping.  `estorides_ui.css` enforces `[hidden] { displ
- Language: py
- Symbols:
...
- Layer: testing
- Doc: ATDD + BDD tests for estorides_core.vuln_correlation.  Implements the Given-When-Then contracts declared in ``spec/vuln_
- Language: py
- Symbols:
...
- Layer: testing
- Doc: BDD tests for the web-layer decorators/helpers extracted from `create_app`.  - WH1: `_provides` returns 503 when the ser
- Language: py
- Symbols:
