...
| 2026-09-11 | `boy_scout_remediation` (auditoría integral) | `spec/boy_scout_remediation.md` (nuevo) | `tests/test_cli_watch.py`, `tests/test_config_env.py`, `tests/test_parsers.py`, `tests/properties/test_parsers_properties.py`, `tests/test_source_loader.py`, `tests/test_entity_extraction.py`, `tests/test_ui_visibility.py` + regresión en `tests/test_recon_report.py` | Auditoría senior de arquitectura + calidad + boy-scout. **Bugs reales**: `estorides watch add` muerto (`AttributeError` por flags ausentes + `NameError` en `SCHEDULER_ENABLED`); env var malformada crasheaba todo el proceso al importar (`config.py`); **26/54 parsers violaban el contrato de totalidad** (descubierto por fuzzing hypothesis, 1000 ej.) y se blindaron con `_d`/`_list`/`_text`; `recon_report` hardcodeaba `TLP:AMBER` en el executive summary; fuzzy merge elegía el canonical **menos** observado; `types=[]` escaneaba todo; multi-doc YAML se descartaba; nombre duplicado corrompía conteos de categoría; `_diff_section` KeyError. **UI crítica**: `[hidden]{display:none!important}` + JS con `style.display` dejaba 8 superficies (progress, tooltip, inspector, context menu, timeline, onboarding, kbd-help, discover-progress) permanentemente invisibles → helper `setVisible()` que limpia el atributo; XSS en `renderEntities` (type/source) y detalle de fusión (intel_level/lists). **Seguridad**: SHA1 con `usedforsecurity=False`, nosec justificados (B310/B324/B608), `ESTORIDES_PASSIVE_ONLY`/`ESTORIDES_PARALLEL` documentados y ahora sí consumidos. **DRY**: `_flat`/`_first` muertos eliminados, constantes muertas fuera, blobs CSS malformados/`--err` corregidos. Bandit 0 High/Medium; 816→861 verdes (+45). Backlog priorizado (God `create_app`, `Orchestrator.run`, base SQLite compartida) en el spec. |
| 2026-09-11 | `boy_scout_remediation` (round 2 — backlog aplicado) | `spec/sqlite_store.md`, `spec/ids.md`, `spec/envutil.md` (nuevos) | `tests/test_sqlite_store.py`, `tests/test_ids.py`, `tests/test_envutil.py`, `tests/test_web_helpers.py`, `tests/test_hardening.py`, `tests/test_scope.py`, `tests/test_async_client.py`, `tests/test_opsec_contact.py`, `tests/test_structured_extraction.py` | Se aplicó el backlog completo salvo blueprints/mypy masivo. **DRY**: base `SqliteStore` compartida por los 4 stores (se borraron `__init__`/`_init_schema`/`_tx`/`close` duplicados) + `DictMixin`; `ids.stable_id` unifica los 6 sha1[:16]; `reliability_weight*` centraliza los 3 mapas duplicados; `envutil` unifica `_env_int/float/bool`. **Web**: decorador `_provides()` elimina 39 guards `503` inline; `_sse_response()` unifica 3 bloques SSE; `CaseStore.set_notes()` acaba con el acceso a `_conn` privado; se tapan 2 fugas CWE-209 (analyze stream / tool install). **Orquestador**: `intel_resolver.resolve` sale del event loop (`asyncio.to_thread`); `_execute_source` despacha a `_run_system_app`/`_run_http_source`. **Bug real extra**: `with sqlite3.connect(...)` no cierra la conexión → `ResponseCache._conn` cierra siempre (era un leak por lectura/escritura de caché). **Tests**: 7 validadores root (`_test_*.py` + `_validate.py`) portados a pytest/CI; duplicados borrados. **CI**: ruff+bandit cubren `estorides_cli.py` y `estorides_llm/`. 861→920 verdes (+59). bandit 0 High/Medium. |
| 2026-09-18 | `osint_search_boost` (M0-M6) | `spec/orchestrator_fanout.md`, `spec/query_intent.md`, `spec/source_routing.md`, `spec/keyless_sources.md`, `spec/tool_doctor.md`, `spec/web_tools_blueprint.md`, `spec/central_config.md` (nuevos) | `tests/test_orchestrator_fanout.py`, `tests/test_query_intent.py`, `tests/test_source_routing.py`, `tests/test_keyless_sources.py`, `tests/test_tool_doctor.py`, `tests/test_web_tools_blueprint.py`, `tests/test_central_config.py` | Mejora integral de búsqueda OSINT con APIs públicas gratuitas. **M0**: trío duplicado en `orchestrator.py` eliminado (`_extract_cursor`/`_infer_relationships`/`_write_dataset` x2→x1, -31 líneas), `_normalise_results` nuevo (cada task lleva su propio source, adiós `zip(raw_results,targets)` desalineado en el path de deadline), `on_source_done` llega a `system_app`. **M1**: `detect_query_type` v2 (`phone`, `mac`, `@handle`→username, btc bech32 case-insensitive) + `normalize_query` (URL→host, `@h`→`h`). **M2**: `_expand_query_type` direccional (url→domain, hash→md5/sha1/sha256, ip/btc/eth/phone/handle) + `kali_phoneinfoga`/`kali_phonefy` reetiquetados `username`→`phone`. **M3**: 3 YAMLs keyless nuevas (`bgpview_ip`, `bgpview_asn`, `cisa_kev_recent`, 143→146 fuentes) + `parse_bgpview`/`parse_cisa_kev` totales. **M4**: `tool_install.doctor()` (19 binarios system_app + 54 recetas + PATH en un solo informe) + `GET /api/tools/doctor`. **M5**: primer slice del God `create_app` (1723→~1640): `estorides_web_tools.tools_bp` con las 4 rutas tools, mismas URLs/auth/rate-events. **M6**: `RUN_DEADLINE`/`LLM_BACKSTOP_S`/`TOOL_INSTALL_*`/`TEMPLATE_ENVS` a `config.py` vía `envutil` (una env malformada ya no mata el import); `run()` defaults = `HTTP_TIMEOUT`/`RUN_DEADLINE`. 885 verdes (suite no-properties); properties (12 módulos) y mutmut no ejecutables en este entorno (venv sin `hypothesis`/`pytest`/`mutmut`, idéntico en árbol sin cambios — pre-existing). ruff limpio en archivos nuevos; 0 violaciones nuevas (errores restantes en `orchestrator`/`parsers`/`entity_extraction` pre-existentes, verificados vía stash). |
